...
from utils.blogs_crud import create_blog, delete_blog, get_blog_by_id, get_blogs, update_blog
...
def create_new_blog(blog: BlogCreate, db: Session = Depends(connect_db), current_user: User = Depends(get_current_user)):
...
def update_existing_blog(blog_id: UUID, blog: BlogUpdate, db: Session = Depends(connect_db), current_user: User = Depends(get_current_user)):
    db_blog = update_blog(db=db, blog_id=blog_id, blog=blog, user_id=current_user.user_id)
...
def delete_existing_blog(blog_id: UUID, db: Session = Depends(connect_db), current_user: User = Depends(get_current_user)):
