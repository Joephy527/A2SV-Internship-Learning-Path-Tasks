...
async def create_blog_tag(
    blog_id: UUID,
    blog_tag_data: BlogTagCreate,
    db: Session = Depends(connect_db),
    current_user: User = Depends(get_current_user),
):
    blog = db.query(Blog).filter(Blog.blog_id == blog_id).first()
    db_blog_tag = (
        db.query(schema.BlogTag)
        .filter(
            schema.BlogTag.blog_id == blog_id,
            schema.BlogTag.tag_name == blog_tag_data.tag_name,
        )
        .first()
    )
    ...
    if db_blog_tag:
        raise HTTPException(
            ...
            detail="Tag has already been added fot this blog",
        )
    ...
    if user_id != current_user.user_id:
        raise HTTPException(
            ...
            detail="Only the owner of the blog can create blog tags",
        )
...
async def update_blog_tag(
    blog_id: UUID,
    blog_tag_data: BlogTagUpdate,
    db: Session = Depends(connect_db),
    current_user: User = Depends(get_current_user),
):
    blog = db.query(Blog).filter(Blog.blog_id == blog_id).first()
    db_tag = get_tag_by_tag_name(db=db, tag_name=blog_tag_data.tag_name)
    db_updated_tag_name = get_tag_by_tag_name(
        db=db, tag_name=blog_tag_data.updated_tag_name
    )

    if not db_tag or not db_updated_tag_name:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"{blog_tag_data.tag_name} or {blog_tag_data.updated_tag_name} is not a valid tag",
        )
    ...
    if user_id != current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the owner of the blog can update blog tags",
        )

    updated_blog_tag = update_a_blog_tag(
        db=db, blog_id=blog_id, blog_tag_data=blog_tag_data
    )

    if not updated_blog_tag:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="The blog doesn't have this tag",
        )
...
async def delete_blog_tag(
    ...
    if user_id != current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the owner of the blog can delete blog tags",
        )

    deleted_blog_tag = delete_a_blog_tag(
        db=db, blog_id=blog_id, tag_name=tag_data.tag_name
    )
